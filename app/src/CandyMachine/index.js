...
const CandyMachine = ({ walletAddress }) => {
  ...
  const mintToken = async () => {
	...
	const getCandyMachineState = useCallback( async () => {
	  ...
	  // Render go live data in a person's local timezone
    ...
    // Display go live date in EST
    const displayGoLiveDateTimeString = `${new Date(
      goLiveData * 1000
    ).toGMTString()}`

	  // Add this data to the state to render
	  setCandyMachine({
  ...
  const memoizedGetCandyMachineState = React.memo(getCandyMachineState)
              ...
              <div className='gif-grid'> {
                    (mints.map(nftObj => {
                          return  <div className="gif-item" key={nftObj.name}>
                                    <img src={nftObj.imageLink} alt={'Minted NFT {nftObj.name}'} ></img>
                                  </div>
                        }))
                    }
              </div>
  ...
  const mintName = mints.name
  const mintLink = mints.link
  console.log({
    mintName,
    mintLink,
    mints,
  ...
  const renderDropTimer = () => {
    // Get the current date and dropDate in JavaScript Date object
    ...
    if (currentDate > dropDate) {
      console.log('Past drop date!');
    }
  };

  console.log(mints.length > 0)

  return (
    candyMachine && candyMachine.state && (
      <div className="machine-container">
        {/* Add the dropTimer at the beginning of our component */}
        {renderDropTimer()}
          <p>Drop Date: {candyMachine.state.displayGoLiveDateTimeString}</p>
